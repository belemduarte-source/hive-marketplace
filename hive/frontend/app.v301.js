...
const translations = {
  ...
  es: {
    ...
    dpDirectionsTitle:"Cómo llegar", routeLocating:"Obteniendo su ubicación…", routeNoLocation:"No se pudo obtener su ubicación.", routeError:"No se pudo calcular la ruta.", routeGps:"Iniciar GPS", routeStepsBtn:"Pasos", routeArrived:"¡Ha llegado a su destino!", navMsgs:"Mensajes", nearMe:"Cerca de mí", sortDist:"Distancia", sortRating2:"Valoración", sortName2:"Nombre", sheetHint:"Toque \"Ver lista\" para ver las empresas", refreshing:"Actualizando…", regPortfolio:"Portafolio de trabajos", regPortfolioPick:"Añadir fotos…", regPortfolioHint:"Hasta 6 fotos de obras terminadas. Se comprimen automáticamente y aparecen en la ficha.", regPortfolioMax:"Máximo 6 fotos", dpNoEmail:"Esta empresa aún no tiene email registrado — contacte por teléfono o por su web.", navMenu:"Menú", proofCountries:"países", proofSpecialties:"especialidades", exploreByCountry:"Explorar por país", dpMoreInfo:"Más información",
...
let currentLang = (function() {
  try {
    const saved = localStorage.getItem('hive_lang');
    if (saved) return saved;
    // Visitantes que já cá estiveram (têm o id de visita) mantêm o PT de
    // sempre — o idioma automático só se aplica a PRIMEIRAS visitas.
    if (localStorage.getItem('hivex_vid')) return 'pt';
    const nav = (navigator.language || 'pt').slice(0, 2).toLowerCase();
    return ['pt', 'en', 'fr', 'es', 'de'].includes(nav) ? nav : 'en';
  } catch (_) { return 'pt'; }
})();
    ...
    requestAnimationFrame(() => {
      if (map) {
        ...
        if (!window._mapEverShown) {
          ...
          const _sv = (typeof _savedMapView === 'function') ? _savedMapView() : null;
          if (_sv) {
            // Enquadramento da última visita vence o padrão e o foco por país
            map.setView([_sv.lat, _sv.lng], _sv.z);
            currentMapCenter = [_sv.lat, _sv.lng];
            try { invalidateDistanceCache(); } catch (_) {}
            try { if (radiusCircle) radiusCircle.setLatLng([_sv.lat, _sv.lng]); } catch (_) {}
            _visitorFocusDone = true;
          } else {
            map.setView(PORTUGAL_CENTER, 7);
            // Catálogo internacional: se o visitante é de outro país com
            // empresas suficientes, o mapa abre lá (Portugal é o fallback)
            try { _maybeFocusVisitorCountry(); } catch (_) {}
          }
...
function heroSearch() {
  // Mobile: uma só caixa. Encaminha para a pesquisa unificada do mapa, que já
  // distingue cidade (geocode) de palavra-chave — sem os 3 modos do desktop.
  if (window.matchMedia && window.matchMedia('(max-width:768px)').matches) {
    const val = ((document.getElementById('heroLocation') || {}).value
      || (document.getElementById('heroCompanyName') || {}).value
      || (document.getElementById('heroService') || {}).value || '').trim();
    showTab('search');
    const u = document.getElementById('searchUnified');
    if (u) u.value = val;
    try { handleUnifiedSearch(val); } catch (_) {}
    try { applyFilters.now(); } catch (_) {}
    setTimeout(() => { try { openResultsSheet(); } catch (_) {} }, 350);
    return;
  }
  const wrap    = document.getElementById('heroSearchWrap');
...
function updateLandingStats() {
  ...
  // Prova social do hero: números reais quando o catálogo está carregado
  try {
    const pc = document.getElementById('proofCompanies');
    const pn = document.getElementById('proofCountries');
    if (pc && companies.length >= 200) pc.textContent = companies.length.toLocaleString('pt-PT');
    if (pn) {
      const uniq = new Set(companies.map(c => (c.country || 'pt').toLowerCase()));
      if (uniq.size > 5) pn.textContent = uniq.size;
    }
  } catch (_) {}
...
// Comparação só faz sentido dentro da MESMA área de atividade — a interseção
// dos setores (principal + array) de todas as selecionadas não pode ser vazia.
function _cmpSetoresDe(c) {
  const s = new Set();
  if (c && c.sector) s.add(c.sector);
  ((c && c.sectors) || []).forEach(x => s.add(x));
  return s;
}
function _cmpMesmaArea(ids) {
  const list = [...ids].map(id => companies.find(x => Number(x.id) === Number(id))).filter(Boolean);
  if (list.length < 2) return true;
  let inter = _cmpSetoresDe(list[0]);
  for (const c of list.slice(1)) {
    const s2 = _cmpSetoresDe(c);
    inter = new Set([...inter].filter(x => s2.has(x)));
...
function toggleCompare(id) {
  ...
  if (_compareIds.has(id)) {
    ...
    if (!_cmpMesmaArea([..._compareIds, id])) {
      showToast('⚠ ' + t('cmpSameArea'));
      _renderCompareBar();
      _syncCompareCheckboxes();
      return;
    }
  ...
  Object.entries(SECTOR_HIERARCHY).forEach(([groupKey, group]) => {
    ...
    Object.entries(group.categories).forEach(([catKey, cat]) => {
      ...
      Object.entries(cat.subcategories).forEach(([subKey, subLabel]) => {
        ...
        dropdownHTML += `<div class="cat-dd-item${isActive}" data-sector="${subKey}"><span class="cat-dd-label">${tLabel}</span>${cnt > 0 ? ` <span class="cat-dd-count">${cnt}</span>` : ''}</div>`;
      });
    });
    dropdownHTML += `</div>`;
...
function mspOpenSheet(type) {
  ...
  if (type === 'area') {
    ...
    title.textContent = t('mspMore') || 'Mais filtros';
    const sortCur = document.getElementById('sortRating')?.checked ? 'rating'
      : document.getElementById('sortReviews')?.checked ? 'reviews'
      : document.getElementById('sortName')?.checked ? 'name' : 'all';
    const ratingCur = ([...document.querySelectorAll('input[name="minRating"]')].find(r => r.checked) || {}).value || '';
    ...
    const sopts = [['all', 'Todos'], ['rating', 'Melhor avaliação'], ['reviews', '💬 Mais avaliados'], ['name', 'A–Z']];
    const ropts = [['', 'Qualquer'], ['4.8-5', '★★★★★ 4.8–5.0'], ['4.5-4.8', '★★★★★ 4.5–4.8'], ['4-4.5', '★★★★☆ 4.0–4.5'], ['3-4', '★★★★☆ 3.0–4.0'], ['0-3', '★★★☆☆ 0–3.0']];
    body.innerHTML =
      '<div class="msp-sheet-section-label">' + (t('sortBy') || 'Ordenar') + '</div><div class="msp-sheet-row">' +
      sopts.map(([v, l]) => `<button class="msp-sheet-btn${v === sortCur ? ' active' : ''}" onclick="mspSetSort('${v}');mspUpdateChips()">${l}</button>`).join('') + '</div>' +
      '<div class="msp-sheet-section-label">' + (t('minRating') || 'Avaliação') + '</div><div class="msp-sheet-row">' +
      ropts.map(([v, l]) => `<button class="msp-sheet-btn${v === ratingCur ? ' active' : ''}" onclick="mspSetRating('${v}');mspUpdateChips()">${l}</button>`).join('') + '</div>' +
      '<div class="msp-sheet-section-label">Destaque</div><div class="msp-sheet-row">' +
      `<button class="msp-sheet-btn${topRated ? ' active' : ''}" onclick="mspToggleStatus('topRated');this.classList.toggle('active')">⭐ Top</button>` +
      `<button class="msp-sheet-btn${verified ? ' active' : ''}" onclick="mspToggleStatus('verified');this.classList.toggle('active')">✅ Verificado</button>` +
      `<button class="msp-sheet-btn${isNew ? ' active' : ''}" onclick="mspToggleStatus('new');this.classList.toggle('active')">🆕 Novo</button>` +
      '</div>';
...
function mspUpdateChips() {
  ...
  // Chip "Mais" (fusão de Ordenar + Avaliação + Destaque)
  const moreChip = document.getElementById('mspChipMore');
  const moreLabel = document.getElementById('mspChipMoreLabel');
  if (moreChip) {
    const moreCount = (sortActive ? 1 : 0) + (ratingActive ? 1 : 0) + statusCount;
    moreChip.classList.toggle('active', moreCount > 0);
    if (moreLabel) moreLabel.textContent = moreCount > 0 ? (t('mspMore') || 'Mais') + ' (' + moreCount + ')' : (t('mspMore') || 'Mais');
  }
...
function handleUnifiedSearch(val) {
  try { _renderSearchSuggest(val); } catch (_) {}
...
function getFiltered() {
  ...
  for (let i = 0; i < companies.length; i++) {
    ...
    // filtro por país (dropdown Explorar por país)
    if (window._countryFilter && (c.country || 'pt').toLowerCase() !== window._countryFilter) continue;
...
function _runApplyFilters() {
  ...
  // Leitores de ecrã: anuncia a contagem quando os filtros mudam
  try {
    const _al = document.getElementById('ariaLive');
    if (_al) _al.textContent = filtered.length + ' ' + t('resultsCount');
  } catch (_) {}
...
function clearAllFilters() {
  ...
  // elementos opcionais (alguns saíram do markup em simplificações) — nunca rebentar
  const _chk = (id, v) => { const el = document.getElementById(id); if (el) el.checked = v; };
  _chk('filterTopRated', false); _chk('filterVerified', false); _chk('filterNew', false);
  _chk('ratingAny', true); _chk('sortAll', true);
  const _uniInput = document.getElementById('searchUnified'); if(_uniInput) _uniInput.value = '';
  const _rs = document.getElementById('radiusSlider'); if (_rs) _rs.value = 100;   // "Todo o país"
...
function _initResultsSheetDrag() {
  ...
  handle.addEventListener('pointerdown', (e) => {
    ...
    peek = 64;   // fechado = só a barra (CSS translateY(calc(100% - 64px)))
...
function renderNearbyPanel(filtered) {
  ...
  try { renderNearbyChips(); renderCountryPills(); } catch (_) {}
  _nearbyData = filtered.map(c => ({ c, d: calculateDistance(refLat, refLng, c.lat, c.lng) }))
                        .sort((a, b) => ((b.c.featured ? 1 : 0) - (a.c.featured ? 1 : 0)) || (_nearbySort === 'rating' ? ((b.c.rating || 0) - (a.c.rating || 0)) : (a.d - b.d)));
  listEl.innerHTML = '';
  _nearbyRendered = 0;
...
// public OSRM demo server (no API key). Modo GPS: indicações passo-a-passo
// (steps=true) + botão "Iniciar GPS" que abre a navegação nativa.
...
function clearRoute() {
  ...
  if (rs) { rs.hidden = true; rs.innerHTML = ''; }
  const tg = document.getElementById('routeStepsToggle');
  if (tg) { tg.hidden = true; tg.setAttribute('aria-expanded', 'false'); }
}
window.clearRoute = clearRoute;

// Sintetiza a instrução de cada manobra OSRM em PT (o OSRM devolve só
// type/modifier/nome da via, não devolve texto)
function _stepText(s) {
  const n = (s.name || '').trim();
  const via = n ? ' por ' + n : '';
  const em = n ? ' para ' + n : '';
  const mod = {
    'left': 'à esquerda', 'right': 'à direita',
    'slight left': 'ligeiramente à esquerda', 'slight right': 'ligeiramente à direita',
    'sharp left': 'acentuadamente à esquerda', 'sharp right': 'acentuadamente à direita',
    'straight': 'em frente', 'uturn': 'inversão de marcha',
  }[(s.maneuver && s.maneuver.modifier) || ''] || '';
  switch ((s.maneuver && s.maneuver.type) || '') {
    case 'depart': return 'Siga' + (via || ' em frente');
    case 'arrive': return 'Chegou ao destino' + (_routeDest && _routeDest.name ? ': ' + _routeDest.name : '');
    case 'turn': return mod === 'inversão de marcha' ? 'Faça inversão de marcha' + em : 'Vire ' + (mod || '') + em;
    case 'continue': case 'new name': return 'Continue ' + (mod && mod !== 'em frente' ? mod : 'em frente') + via;
    case 'roundabout': case 'rotary': case 'roundabout turn':
      return 'Na rotunda, apanhe a ' + ((s.maneuver && s.maneuver.exit) || 1) + '.ª saída' + em;
    case 'merge': return 'Entre' + (em || ' na via');
    case 'on ramp': return 'Apanhe o acesso' + em;
    case 'off ramp': return 'Saia' + em;
    case 'end of road': return 'No fim da estrada, vire ' + (mod || '') + em;
    case 'fork': return 'Na bifurcação, mantenha-se ' + (mod || 'em frente') + em;
    default: return 'Continue' + via;
  }
}

function _renderRouteSteps(route) {
  const rs = document.getElementById('routeSteps');
  const tg = document.getElementById('routeStepsToggle');
  if (!rs || !tg) return;
  const steps = (route.legs && route.legs[0] && route.legs[0].steps) || [];
  if (steps.length < 2) { tg.hidden = true; rs.hidden = true; return; }
  const distStr = m => m >= 1000
    ? ((m / 1000) < 10 ? (m / 1000).toFixed(1).replace('.', ',') : Math.round(m / 1000)) + ' km'
    : Math.round(m) + ' m';
  rs.innerHTML = steps.map((s, i) =>
    '<div class="route-step"><span class="route-step-n">' + (i + 1) + '</span>'
    + '<span class="route-step-txt">' + escHtml(_stepText(s)) + '</span>'
    + (s.distance > 1 ? '<span class="route-step-dist">' + distStr(s.distance) + '</span>' : '')
    + '</div>').join('');
  tg.hidden = false;
  rs.hidden = true;   // fechado por defeito; abre no botão "Passos"
...
function openNativeNav() {
  const d = _routeDest || (typeof selectedId !== 'undefined' && selectedId != null
    ? companies.find(x => x.id === selectedId) : null);
  if (!d || d.lat == null) return;
  const o = _routeUserLatLng();
  const url = 'https://www.google.com/maps/dir/?api=1'
    + (o ? '&origin=' + o[0] + ',' + o[1] : '')
    + '&destination=' + d.lat + ',' + d.lng + '&travelmode=driving';
  window.open(url, '_blank', 'noopener');
}
window.openNativeNav = openNativeNav;
...
async function routeToCompany(id) {
  ...
  try {
    const url = `https://router.project-osrm.org/route/v1/driving/`
      + `${origin[1]},${origin[0]};${c.lng},${c.lat}?overview=full&geometries=geojson&steps=true`;
...
function _renderDetailPanel(c) {
  ...
  // Missão & Visão (colhidas do site oficial; só quando declaradas)
  const _mw = document.getElementById('dpMissionWrap');
  const _vw = document.getElementById('dpVisionWrap');
  if (_mw) {
    const tem = c.mission && String(c.mission).trim();
    _mw.style.display = tem ? '' : 'none';
    if (tem) document.getElementById('dpMissionTxt').textContent = String(c.mission).trim();
  }
  if (_vw) {
    const tem = c.vision && String(c.vision).trim();
    _vw.style.display = tem ? '' : 'none';
    if (tem) document.getElementById('dpVisionTxt').textContent = String(c.vision).trim();
  }
  ...
  if (facts) {
    ...
    if (c.business_hours) {
      ...
      try { items.push('<span class="dp-fact">' + _openBadgeHtml(c) + '</span>'); } catch (_) {}
    }
  ...
  if (quoteBar) {
    ...
    const waB = document.getElementById('dpWaBtn');
    if (waB) {
      let wa = null;
      try { wa = typeof _waNumber === 'function' ? _waNumber(c) : null; } catch (_) {}
      waB.style.display = (loggedIn && wa) ? '' : 'none';
    }
...
function shareCompany() {
  const c = companies.find(x => x.id === selectedId);
  if (!c) return;
  // /e/:id serve og:title/description por empresa aos crawlers (pré-visualização
  // rica no WhatsApp/redes) e redireciona humanos para /?company=id
  const shareUrl = location.origin + '/e/' + c.id;
  const text = c.name + ' — ' + c.description + '\n⭐ ' + c.rating + '/5 · ' + c.city;
  if (navigator.share) {
    navigator.share({ title: c.name, text: text, url: shareUrl }).catch(() => {});
  } else {
...
function _openChatOverlay() {
  ...
  const lay = document.querySelector('#inboxOverlay .chat-layout');
  if (lay) lay.classList.remove('thread-open');   // mobile: começa na lista
  _chatLastSig = '';
  _chatEl('inboxOverlay').classList.add('open');
  _chatStartPoll();
}
function closeInbox() { _chatEl('inboxOverlay').classList.remove('open'); _chatStopPoll(); }
...
async function _loadOwnerThreads() {
  try {
    ...
    // o GET marca as lidas DEPOIS do snapshot devolvido — dá para mostrar
    // "novas" por conversa nesta abertura
    _renderThreadList(_chatThreads.map(th => {
      const last = th.items[th.items.length - 1];
      return {
        id: th.key, title: th.name || th.email,
        sub: last ? (last.body || '📄 documento') : th.email,
        unread: th.items.filter(m => m.sender === 'client' && !m.read_at).length,
        initials: (th.name || th.email || '?').trim().charAt(0).toUpperCase(),
        at: last ? last.created_at : null,
      };
    }));
    if (_chatThreads.length && !_chatIsMobile()) _selectOwnerThread(_chatThreads[0].key);
    else if (!_chatThreads.length) _chatEmpty();
...
function _selectOwnerThread(key) {
  ...
  if (!th) return;
  _chatEl('chatHead').innerHTML = '<button class="chat-back" onclick="chatBackToList()" aria-label="Voltar à lista">‹</button>' +
    escHtml(th.name || th.email) +
    ' <span class="chat-head-sub">' + escHtml(th.email) + '</span>';
  _renderMsgs(th.items, 'company', true);
...
async function _loadClientThreads() {
  try {
    ...
    _renderThreadList(rows.map(r => ({
      ...
      emoji: r.company_emoji || null, at: r.last_at || null,
    })));
    if (rows.length && !_chatIsMobile()) _selectClientThread(rows[0].company_id);
    else if (!rows.length) _chatEmpty();
  } catch (e) { _chatError(e); }
}
async function _selectClientThread(companyId) {
  ...
  _chatEl('chatHead').innerHTML = '<button class="chat-back" onclick="chatBackToList()" aria-label="Voltar à lista">‹</button>' +
    escHtml(th ? th.company_name : '') + (th && th.emoji ? ' <span class="chat-head-sub">' + escHtml(th.emoji) + '</span>' : '');
  ...
  try {
    ...
    _renderMsgs(msgs, 'client', true);
    setTimeout(() => { try { _refreshMsgBadge(); } catch (_) {} }, 400);
  } catch (e) { _chatError(e); }
}
...
function _renderThreadList(items) {
  _chatEl('chatThreads').innerHTML = items.map(i => `
    <button class="chat-thread" data-tid="${escHtml(String(i.id))}" onclick="chatSelectThread(this.dataset.tid)">
      <span class="ct-ava">${i.emoji ? escHtml(i.emoji) : escHtml(i.initials || (i.title || '?').charAt(0).toUpperCase())}</span>
      <span class="chat-thread-title">${escHtml(i.title || '')}</span>
      <span class="ct-time">${i.at ? _chatWhen(i.at) : ''}</span>
...
function _chatLocale() {
  return { pt: 'pt-PT', en: 'en-GB', fr: 'fr-FR', es: 'es-ES', de: 'de-DE' }[currentLang] || 'pt-PT';
}
function _chatDayKey(d) { return d.getFullYear() + '-' + d.getMonth() + '-' + d.getDate(); }
function _chatDayLabel(d) {
  const now = new Date();
  if (_chatDayKey(d) === _chatDayKey(now)) return t('chatToday');
  if (_chatDayKey(d) === _chatDayKey(new Date(Date.now() - 86400000))) return t('chatYesterday');
  const opts = { day: 'numeric', month: 'long' };
  if (d.getFullYear() !== now.getFullYear()) opts.year = 'numeric';
  return d.toLocaleDateString(_chatLocale(), opts);
}
// hora curta para a lista de conversas: HH:MM hoje, "Ontem", senão dd/mm
function _chatWhen(iso) {
  const d = new Date(iso);
  if (isNaN(d)) return '';
  if (_chatDayKey(d) === _chatDayKey(new Date())) return d.toLocaleTimeString(_chatLocale(), { hour: '2-digit', minute: '2-digit' });
  if (_chatDayKey(d) === _chatDayKey(new Date(Date.now() - 86400000))) return t('chatYesterday');
...
function _renderMsgsInner(msgs, mySender, forceBottom) {
  ...
  box.innerHTML = msgs.map(m => {
    ...
    let sep = '';
    if (_chatDayKey(dt) !== lastDay) {
      lastDay = _chatDayKey(dt);
      sep = '<div class="chat-day"><span>' + escHtml(_chatDayLabel(dt)) + '</span></div>';
    }
    const mine = m.sender === mySender;
    const ticks = mine ? `<span class="chat-ticks${m.read_at ? ' read' : ''}" title="${m.read_at ? 'Lida' : 'Enviada'}">${m.read_at ? '✓✓' : '✓'}</span>` : '';
    const files = (m.files || []).map(f => {
      const isImg = /^image\//.test(f.mime || '');
      return isImg
        ? `<button class="chat-file" data-fid="${f.id}" data-fname="${escHtml(f.name)}" onclick="chatPreviewImage(this.dataset.fid,this.dataset.fname)" title="Ver imagem">🖼️ ${escHtml(f.name)} <span>${_fmtSize(f.size)}</span></button>`
        : `<button class="chat-file" data-fid="${f.id}" data-fname="${escHtml(f.name)}" onclick="chatDownload(this.dataset.fid,this.dataset.fname)" title="Transferir documento">📄 ${escHtml(f.name)} <span>${_fmtSize(f.size)}</span></button>`;
    }).join('');
    return sep + `<div class="chat-row ${mine ? 'mine' : ''}"><div class="chat-bubble">
      ${m.body ? `<div class="chat-text">${escHtml(m.body)}</div>` : ''}
      ${files ? `<div class="chat-file-list">${files}</div>` : ''}
      <div class="chat-meta">${dt.toLocaleTimeString(_chatLocale(), { hour: '2-digit', minute: '2-digit' })}${ticks}</div>
    </div></div>`;
...
function openProfilePanel() {
  ...
  // A conta admin é dona de milhares de fichas importadas — construir um
  // cartão DOM para todas congelava/crashava o browser. A lista é agora
  // paginada (15 + "Mostrar mais") com filtro, e as estatísticas 30d só se
  // pedem para os cartões visíveis.
  window._profMyCompanies = myCompanies;
  window._profCoShown = 15;
  window._profCoFilter = '';
  setTimeout(() => { try { _renderMyAlerts(); } catch (_) {} }, 90);

  const listControls = myCompanies.length > 0
    ? `${myCompanies.length > 15 ? `<input id="profCoFilter" type="search" placeholder="${escHtml(t('profFilterPh'))}" oninput="profCoFilterInput(this.value)" style="width:100%;margin-bottom:10px;background:var(--bg);border:1.5px solid var(--border);border-radius:10px;padding:10px 13px;font-size:13.5px;color:var(--text);font-family:inherit"/>` : ''}
      <div id="profCoList"></div>
      <div id="profCoMore" style="text-align:center;margin-top:10px"></div>`
    : '';
  ...
  document.getElementById('profilePanelBody').innerHTML = `
    ...
    <div style="margin-top:6px">
      ...
      ${listControls || `<div class="profile-empty" style="text-align:center;padding:28px 16px;color:var(--muted)">
      ...
      const cardsHtml = reviews.map(r => {
        ...
        return `<div class="review-card" id="rc-${r.id}">
          <div class="review-card-top">
            <span class="review-card-author">${escHtml(r.author_name)}${r.verified_contact ? ' <span class="rv-verified" title="Contactou esta empresa através da Hivex">✓ ' + escHtml(t('rvVerified')) + '</span>' : ''}</span>
...
/* ══════════════════════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════════════════════ */
// (dropdown de destacadas removido — o destaque vive nas listas e no Início)
...
function renderLpFeatured() {
  ...
  const item = c => {
    ...
    if (c.email) acoes.push('<a class="fc-act" href="mailto:' + escHtml(c.email) + '" onclick="event.stopPropagation()">✉️ Email</a>');
    if (c.website) acoes.push('<a class="fc-act" href="' + escHtml(c.website) + '" target="_blank" rel="noopener" onclick="event.stopPropagation()">🌐 Site</a>');
...
/* ══ DESKTOP INTERNACIONAL (1/2): foco no país do visitante + pílulas ══ */
const _COUNTRY_VIEWS = {
  pt:[39.5,-8.0,7], es:[40.2,-3.7,6], fr:[46.6,2.4,6], de:[51.2,10.4,6],
  uk:[54.0,-2.5,6], it:[42.5,12.5,6], nl:[52.2,5.3,7], be:[50.6,4.5,8],
  ch:[46.8,8.2,8], at:[47.6,14.1,7], ie:[53.2,-8.0,7], se:[62.0,15.0,5],
  no:[64.5,12.0,5], dk:[56.0,10.0,7], fi:[64.0,26.0,5], pl:[52.1,19.4,6],
  cz:[49.8,15.5,7], gr:[38.5,23.5,7], hu:[47.2,19.5,7], ro:[45.9,25.0,7],
  hr:[45.1,16.4,7], sk:[48.7,19.7,8], si:[46.1,14.8,8], lt:[55.3,23.9,7],
  lv:[56.9,24.6,7], ee:[58.7,25.5,7], lu:[49.8,6.1,9], bg:[42.7,25.3,7],
  us:[39.8,-98.5,4], ca:[56.1,-106.3,4], mx:[23.6,-102.5,5],
};
const _FLAG_CC = { uk: 'gb' };
function _flagEmoji(cc) {
  const c = (_FLAG_CC[cc] || cc).toUpperCase();
  return String.fromCodePoint(...[...c].map(ch => 0x1F1A5 + ch.charCodeAt(0)));
}
function _visitorCC() {
  if (window._userCountry && _COUNTRY_VIEWS[window._userCountry]) return window._userCountry;
  try {
    const region = (navigator.language || '').split('-')[1];
    if (region) {
      let cc = region.toLowerCase();
      if (cc === 'gb') cc = 'uk';
      if (_COUNTRY_VIEWS[cc]) return cc;
    }
  } catch (_) {}
  return null;
}
function _countryCounts() {
  const by = {};
  (companies || []).forEach(c => { const k = (c.country || 'pt').toLowerCase(); by[k] = (by[k] || 0) + 1; });
  return by;
}
function flyToCountry(cc) {
  const v = _COUNTRY_VIEWS[cc];
  ...
  try { invalidateDistanceCache(); } catch (_) {}
  try { if (radiusCircle) radiusCircle.setLatLng([v[0], v[1]]); } catch (_) {}
  setTimeout(() => { try { applyFilters.now(); } catch (_) { applyFilters(); } }, 250);
}
window.flyToCountry = flyToCountry;

let _visitorFocusDone = false;
function _maybeFocusVisitorCountry() {
  if (_visitorFocusDone) return;
  const cc = _visitorCC();
  if (!cc || cc === 'pt') { _visitorFocusDone = true; return; }
  const n = _countryCounts()[cc] || 0;
  if (n < 10) return;
  if (typeof selectedId !== 'undefined' && selectedId) { _visitorFocusDone = true; return; }
  _visitorFocusDone = true;
  flyToCountry(cc);
  try { showToast(_flagEmoji(cc) + ' ' + n + ' ' + t('resultsCount')); } catch (_) {}
}
window._maybeFocusVisitorCountry = _maybeFocusVisitorCountry;
...
function renderCountryPills() {
  ...
  const sel = window._countryFilter;
  const rotulo = sel
    ? _flagEmoji(sel) + ' <span>' + escHtml(nome(sel)) + '</span>'
    : '🌍 <span>' + escHtml(t('exploreByCountry')) + '</span>';
  box.innerHTML =
...
function _buildSuggestIndex() {
  ...
  (companies || []).forEach(c => {
    ...
    if (!cities.has(k)) cities.set(k, { label: city, n: 0, lat: c.lat, lng: c.lng });
    cities.get(k).n++;
  });
  const tr = translations[currentLang] || translations.pt;
  const sectors = Object.entries(tr.sectors || {}).map(([k, label]) => ({ k, label }));
  _suggestIndex = { cities: [...cities.values()], sectors };
}
const _normTxt = s => String(s).toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '');
function _renderSearchSuggest(val) {
  ...
  if (!box) {
    ...
    const wrap = input.closest('.ub-input') || input.parentElement;
  ...
  let sectors = _suggestIndex.sectors.filter(s => _normTxt(s.label).includes(q)).slice(0, 2);
  let comps = (companies || []).filter(c => _normTxt(c.name).includes(q)).slice(0, 3);
  if (!cities.length && !sectors.length && !comps.length && q.length >= 4 && typeof fuzzyMatchToken === 'function') {
    // Tolerância a gralhas ("eletrecista", "canalizaçao") — só quando o exato falha
    const fz = v => { try { return fuzzyMatchToken(q, _normTxt(v)); } catch (_) { return false; } };
    cities = _suggestIndex.cities.filter(c => fz(c.label)).sort((a, b) => b.n - a.n).slice(0, 3);
  ...
  box.innerHTML =
    ...
    comps.map(c => '<div class="ss-item" onmousedown="suggestPickCompany(' + c.id + ')"><span class="ss-ico">🏢</span>' + escHtml(c.name) + '</div>').join('');
  box.style.display = 'block';
}
...
function suggestPickCity(label, lat, lng) {
  ...
  _keywordFilter = '';
  if (typeof map !== 'undefined' && map) { map.setView([lat, lng], 11, { animate: true }); currentMapCenter = [lat, lng]; }
  try { invalidateDistanceCache(); } catch (_) {}
  try { if (radiusCircle) radiusCircle.setLatLng([lat, lng]); } catch (_) {}
  applyFilters();
}
...
window._buildSuggestIndex = _buildSuggestIndex;

let _nearbySort = 'dist';
function setNearbySort(k) {
  _nearbySort = k;
  document.querySelectorAll('.nearby-sort button').forEach(b => b.classList.toggle('active', b.dataset.ns === k));
...
/* ══ LOTE UX v242: vista do mapa persistida + badge não-lidas + teclado ══ */
function _savedMapView() {
  try {
    const v = JSON.parse(localStorage.getItem('hivex_map_view') || 'null');
    if (v && isFinite(v.lat) && isFinite(v.lng) && isFinite(v.z) &&
        Math.abs(v.lat) <= 90 && Math.abs(v.lng) <= 180) return v;
  } catch (_) {}
  return null;
}
(function () {
  ...
  map.on('moveend zoomend', () => {
    ...
    _mvT = setTimeout(() => {
      try {
        ...
        localStorage.setItem('hivex_map_view', JSON.stringify({ lat: c.lat, lng: c.lng, z: map.getZoom() }));
      } catch (_) {}
    }, 400);
  });
})();

// Badge de não-lidas: soma conversas como cliente + inbox das empresas do dono.
// Alvos: .nav-msgs (bottom nav mobile) e .avatar-wrap (desktop, sessão iniciada).
async function _refreshMsgBadge() {
  const logged = !!localStorage.getItem('hive_user');
  let n = 0;
  if (logged) {
    try { const r = await apiFetch('/messages/unread-total'); n = (r && r.n) || 0; } catch (_) { return; }
  }
  document.querySelectorAll('.nav-msgs, .avatar-wrap').forEach(b => {
    let dot = b.querySelector('.nav-unread');
    if (n > 0) {
      if (!dot) { dot = document.createElement('span'); dot.className = 'nav-unread'; b.appendChild(dot); }
      dot.textContent = n > 99 ? '99+' : String(n);
    } else if (dot) dot.remove();
  });
}
window._refreshMsgBadge = _refreshMsgBadge;
// updateNavAuth reconstrói a área de autenticação — reanexa o badge a seguir
// (nota: já há outro wrapper em volta de updateNavAuth; este embrulha o atual)
const _preBadgeUpdateNavAuth = updateNavAuth;
updateNavAuth = function () {
  _preBadgeUpdateNavAuth.apply(this, arguments);
  try { _refreshMsgBadge(); } catch (_) {}
};
setTimeout(() => { try { _refreshMsgBadge(); } catch (_) {} }, 2500);
setInterval(() => { try { if (localStorage.getItem('hive_user')) _refreshMsgBadge(); } catch (_) {} }, 120000);

// Enter/Espaço abrem o cartão de proximidade focado (tab navega entre eles)
document.addEventListener('keydown', (e) => {
  if (e.key !== 'Enter' && e.key !== ' ') return;
  const card = e.target && e.target.closest ? e.target.closest('.nearby-card[tabindex]') : null;
  if (card) { e.preventDefault(); card.click(); }
});


/* ══ CHAT v244: atualização ao vivo + painel mobile + imagens + composer ══ */
let _chatPollT = null;
let _chatLastSig = '';
function _chatIsMobile() { return window.innerWidth <= 768; }
function _chatOpenPane() {
  const lay = document.querySelector('#inboxOverlay .chat-layout');
  if (lay) lay.classList.add('thread-open');
}
function chatBackToList() {
  const lay = document.querySelector('#inboxOverlay .chat-layout');
  if (lay) lay.classList.remove('thread-open');
}
window.chatBackToList = chatBackToList;

// Poll a cada 10s enquanto o overlay está aberto e o separador visível —
// re-renderiza apenas quando a conversa mudou (novas mensagens ou ✓✓).
function _chatMsgSig(msgs) {
  const last = msgs.length ? msgs[msgs.length - 1].id : 0;
  return msgs.length + ':' + last + ':' + msgs.reduce((n, m) => n + (m.read_at ? 1 : 0), 0);
}
async function _chatPollTick() {
  const ov = _chatEl('inboxOverlay');
  if (!ov || !ov.classList.contains('open')) { _chatStopPoll(); return; }
  if (document.hidden) return;
  try {
    if (_chatMode === 'client' && _chatCompanyId) {
      const msgs = await apiFetch('/messages/thread/' + _chatCompanyId);
      const sig = _chatMsgSig(msgs);
      if (sig !== _chatLastSig) {
        _chatLastSig = sig;
        _renderMsgs(msgs, 'client');
        try { _refreshMsgBadge(); } catch (_) {}
      }
    } else if (_chatMode === 'owner' && _chatCompanyId && _chatPeer) {
      // nota: este GET marca o inbox como lido — semântica "inbox aberto = visto"
      const all = await apiFetch('/messages/company/' + _chatCompanyId);
      const mine = all.filter(m => (m.client_email || '').toLowerCase() === _chatPeer)
        .sort((a, b) => new Date(a.created_at) - new Date(b.created_at));
      const sig = _chatMsgSig(mine);
      if (sig !== _chatLastSig) {
        _chatLastSig = sig;
        const th = _chatThreads.find(x => x.key === _chatPeer);
        if (th) th.items = mine;
        _renderMsgs(mine, 'company');
        try { _refreshMsgBadge(); } catch (_) {}
      }
    }
  } catch (_) { /* rede instável: tenta no próximo tick */ }
}
function _chatStartPoll() {
  clearInterval(_chatPollT);
  _chatPollT = setInterval(_chatPollTick, 10000);
}
...
// Imagens abrem em lightbox (transferir continua disponível nos outros tipos)
async function chatPreviewImage(fileId, name) {
  try {
    const f = await apiFetch('/messages/file/' + fileId);
    if (!/^image\//.test(f.mime || '')) return chatDownload(fileId, name);
    openLightbox('data:' + f.mime + ';base64,' + f.data);
  } catch (e) {
    showToast(e.message || 'Não foi possível abrir a imagem');
  }
}
window.chatPreviewImage = chatPreviewImage;

// Composer: Enter envia (Shift+Enter = nova linha) + autosize até 4 linhas
(function () {
  const ta = document.getElementById('chatComposeInput');
  if (!ta) return;
  ta.addEventListener('keydown', (e) => {
    if (e.key === 'Enter' && !e.shiftKey) {
      e.preventDefault();
      const btn = document.querySelector('#inboxOverlay .chat-send');
      if (btn && !btn.disabled) chatSend(btn);
    }
  });
...
/* ══ VAGA C: web push + alertas de pesquisa ══ */
...
async function enablePush() {
  try {
    ...
    const { key } = await apiFetch('/push/pubkey');
    ...
    showToast(t('pushOn'));
    const hint = document.getElementById('chatPushHint');
    if (hint) hint.remove();
    return true;
...
// Convite discreto dentro do chat (nunca prompt automático ao carregar o site)
function _renderPushHint() {
  try {
    if (!('Notification' in window) || Notification.permission !== 'default') return;
    if (!localStorage.getItem('hive_user')) return;
    if (document.getElementById('chatPushHint')) return;
    const head = document.querySelector('#inboxOverlay .chat-retention');
    if (!head) return;
    const div = document.createElement('div');
    div.id = 'chatPushHint';
    div.style.cssText = 'margin:6px 0 2px;display:flex;align-items:center;gap:10px;font-size:12.5px;color:var(--text-secondary)';
    div.innerHTML = '🔔 <button onclick="enablePush()" style="background:var(--primary);color:#fff;border:none;border-radius:8px;padding:6px 12px;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit">' + escHtml(t('pushBtn')) + '</button>';
    head.after(div);
  } catch (_) {}
}
// liga o convite à abertura do chat
(function () {
  const orig = _openChatOverlay;
  _openChatOverlay = function () { orig.apply(this, arguments); try { _renderPushHint(); } catch (_) {} };
})();

// Alertas de pesquisa guardados
async function saveSearchAlert() {
  try {
    const sector = [...activeSectors][0];
    if (!sector) { showToast(t('emptySectorTitle')); return; }
    const input = document.getElementById('searchUnified');
    const cityTxt = input && input.value && input.value.trim().length <= 40 ? input.value.trim() : null;
    const country = (typeof _visitorCC === 'function' && _visitorCC()) || 'pt';
    await apiFetch('/saved-searches', { method: 'POST', body: { sector, city: cityTxt, country } });
    showToast(t('alertSaved'));
  } catch (e) { showToast(e.message || 'Erro'); }
}
window.saveSearchAlert = saveSearchAlert;

async function _renderMyAlerts() {
  if (!localStorage.getItem('hive_user')) return;
  let rows = [];
  try { rows = await apiFetch('/saved-searches'); } catch (_) { return; }
  if (!rows.length) return;
  const body = document.getElementById('profilePanelBody');
  if (!body || document.getElementById('myAlertsBox')) return;
  const tr = translations[currentLang] || translations.pt;
  const sc = tr.sectors || {};
  body.insertAdjacentHTML('beforeend',
    '<div id="myAlertsBox" style="margin-top:18px"><div class="profile-section-title">🔔 ' + escHtml(t('alertsTitle')) + '</div>' +
    rows.map(r =>
      '<div style="display:flex;align-items:center;gap:8px;padding:8px 2px;border-bottom:1px solid var(--border);font-size:13px">' +
      '<span style="flex:1">' + escHtml(sc[r.sector] || r.sector) + (r.city ? ' · ' + escHtml(r.city) : '') + (r.country ? ' · ' + escHtml(String(r.country).toUpperCase()) : '') + '</span>' +
      '<button onclick="deleteSearchAlert(' + r.id + ', this)" style="background:none;border:none;color:var(--text-secondary);cursor:pointer;font-size:15px" aria-label="Remover alerta">✕</button></div>').join('') +
    '</div>');
}
async function deleteSearchAlert(id, btn) {
  try {
...
/* ══ PERFIL: lista de empresas paginada (corrige crash com milhares) ══ */
function _profCardHtml(c) {
  return `
...
function _renderProfCoList() {
  const box = document.getElementById('profCoList');
  if (!box) return;
  const all = window._profMyCompanies || [];
  const q = _normTxt(window._profCoFilter || '');
  const filtradas = q
    ? all.filter(c => _normTxt(c.name).includes(q) || _normTxt(c.city || '').includes(q))
    : all;
  const slice = filtradas.slice(0, window._profCoShown || 15);
  box.innerHTML = slice.map(_profCardHtml).join('') ||
    '<div style="text-align:center;padding:18px;color:var(--muted);font-size:13px">—</div>';
  const more = document.getElementById('profCoMore');
  if (more) {
    more.innerHTML = filtradas.length > slice.length
      ? `<div style="font-size:12px;color:var(--text-secondary);margin-bottom:8px">${t('profShowing')} ${slice.length} / ${filtradas.length}</div>
         <button onclick="profCoShowMore()" style="background:var(--bg);border:1.5px solid var(--border);border-radius:10px;padding:10px 22px;font-size:13px;font-weight:700;color:var(--text);cursor:pointer;font-family:inherit">${t('profShowMore')} ↓</button>`
      : '';
  }
  // estatísticas só para os cartões renderizados (cap 8 por página)
  setTimeout(() => { try { _loadMyCompanyStats(slice.slice(0, 8)); } catch (_) {} }, 60);
}
let _profCoFilterT = null;
function profCoFilterInput(v) {
  clearTimeout(_profCoFilterT);
  _profCoFilterT = setTimeout(() => {
    window._profCoFilter = v || '';
    window._profCoShown = 15;
    _renderProfCoList();
  }, 200);
}
function profCoShowMore() {
  window._profCoShown = (window._profCoShown || 15) + 30;
  _renderProfCoList();
}
...
/* ══ Simplificação: menu ⋯ do painel de detalhe ══ */
// NOME PRÓPRIO: toggleDpMore já era o expansor do "Mais informação" — a
// versão homónima do menu ⋯ sobrepunha-o e partia a secção (v256).
function toggleDpMoreMenu(e) {
  if (e) e.stopPropagation();
  const m = document.getElementById('dpMoreMenu');
  if (m) m.classList.toggle('open');
}
window.toggleDpMoreMenu = toggleDpMoreMenu;
document.addEventListener('click', (e) => {
  const m = document.getElementById('dpMoreMenu');
  if (m && m.classList.contains('open') && !e.target.closest('.dp-more-wrap')) m.classList.remove('open');
});


// Limpa o filtro por país (linha 'Todos os países' do dropdown)
...
// ═══════════ RECOMENDAÇÕES: TOP 3 POR ÁREA (scorecard) ═══════════
// Pontuação 0-100 por empresa dentro do raio escolhido:
//   40% proximidade (decay linear até ao raio)
//   30% ficha completa (site 2, email 1.5, tel 1.5, alvará 2, logo 1, descrição 1, verificada 1 → /10)
//   30% avaliações (média bayesiana: prior 3.5★ com peso de 5 avaliações,
//       para que 1×5★ não bata 20×4.8★ nem a ausência de avaliações domine)
const _recsSel = new Set();
let _recsLoc = null;

const _recsEsc = s => String(s == null ? '' : s).replace(/[&<>"']/g, m => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[m]));

function _recsGrupos() {
  const out = [];
  for (const g of Object.values(SECTOR_HIERARCHY)) {
    const grupo = { label: g.label, subs: [] };
    for (const c of Object.values(g.categories || {}))
      for (const [k, lbl] of Object.entries(c.subcategories || {})) grupo.subs.push([k, lbl]);
    if (grupo.subs.length) out.push(grupo);
  }
  return out;
}
function _recsLabel(k) {
  for (const g of Object.values(SECTOR_HIERARCHY))
    for (const c of Object.values(g.categories || {}))
      if (c.subcategories && c.subcategories[k]) return c.subcategories[k];
  ...
  if (!_recsLoc) {
    try {
      if (typeof map !== 'undefined' && map && map.getCenter) {
        const c = map.getCenter();
        _recsLoc = { lat: c.lat, lng: c.lng, label: t('recsMapCenter') };
      }
    } catch (_) {}
  }
...
function _recsPaintCount() {
  ...
  if (n) n.textContent = _recsSel.size ? '(' + _recsSel.size + ')' : '';
}
function recsUseGps() {
...
function recsGeocodeCity() {
  const inp = document.getElementById('recsCity');
...
function _recsFicha(c) {
  ...
  if (c.alvara) p += 2;
  if (c.logo) p += 1;
  if (c.description) p += 1;
  if (c.verified) p += 1;
  return Math.min(1, p / 10);
}
function _recsAval(c) {
  const r = +c.rating || 0, n = +c.reviews || 0;
  return ((r * n + 3.5 * 5) / (n + 5)) / 5;
}
function recsRun() {
  ...
  for (const k of _recsSel) {
    ...
    for (const c of companies) {
      ...
      if (c.sector !== k && !(Array.isArray(c.sectors) && c.sectors.includes(k))) continue;
      const d = _getCachedDistance(_recsLoc.lat, _recsLoc.lng, c);
      if (!isFinite(d) || d > raio) continue;
      const prox = Math.max(0, 1 - d / raio);
      const ficha = _recsFicha(c);
      const aval = _recsAval(c);
      const total = Math.round(100 * (0.4 * prox + 0.3 * ficha + 0.3 * aval));
      cand.push({ c, d, prox, ficha, aval, total });
    }
    cand.sort((a, b) => b.total - a.total || a.d - b.d);
...
function _recsCard(x, i) {
  const c = x.c;
  const medalha = ['🥇', '🥈', '🥉'][i] || '';
  const logo = c.logo
    ? '<img class="recs-logo" src="' + _recsEsc(c.logo) + '" alt="" loading="lazy"/>'
    : '<span class="recs-logo recs-logo-emoji">' + (c.emoji || '🏗️') + '</span>';
  const nAval = +c.reviews || 0;
  const estrelas = nAval
    ? '★ ' + (+c.rating || 0).toFixed(1) + ' <span class="recs-dim">(' + nAval + ')</span>'
    : '';
  const chips = [];
  if (c.website) chips.push('🌐 Site');
  if (c.alvara) chips.push('📜 ' + _recsEsc(c.alvara));
  if (c.phone || c.email) chips.push('📞');
  if (c.verified) chips.push('✔');
  const barra = (lbl, v) =>
    '<div class="recs-bar-row"><span>' + _recsEsc(lbl) + '</span><div class="recs-bar"><i style="width:' + Math.round(v * 100) + '%"></i></div></div>';
  return '<div class="recs-card" onclick="closeRecs();openDetail(' + Number(c.id) + ')">' +
    '<div class="recs-card-top"><span class="recs-medal">' + medalha + '</span>' + logo +
    '<div class="recs-card-nome"><b>' + _recsEsc(c.name) + '</b>' +
    '<div class="recs-dim">' + _recsEsc(c.city || '') + ' · ' + x.d.toFixed(1) + ' km' + (estrelas ? ' · ' + estrelas : '') + '</div></div>' +
    '<div class="recs-donut" style="background:conic-gradient(var(--primary) ' + (x.total * 3.6) + 'deg, var(--border, #e2e8f0) 0)"><b>' + x.total + '</b></div></div>' +
    barra(t('recsProximity'), x.prox) + barra(t('recsProfileScore'), x.ficha) + barra(t('recsReviewsScore'), x.aval) +
    '<div class="recs-chips-mini">' + chips.map(ch => '<span>' + ch + '</span>').join('') + '</div>' +
...
// Mobile: placeholder genérico na caixa do hero (uma só pesquisa)
try {
  if (window.matchMedia('(max-width:768px)').matches) {
    const hl = document.getElementById('heroLocation');
    if (hl) hl.setAttribute('placeholder', 'Cidade, empresa ou atividade…');
  }
} catch (_) {}
